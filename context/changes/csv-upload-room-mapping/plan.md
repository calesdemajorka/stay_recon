...
**Contract**: `RoomRowForm(forms.Form)` with `room_number`, `room_type`, `capacity` — all plain `CharField(required=False)` — a plain (non-model) formset via `forms.formset_factory(RoomRowForm, extra=0)`, one form per row on the current page, used only for GET-time rendering and the submitted `TOTAL_FORMS` count. `csv_preview(request, event_pk)` view, `@login_required`, scoped via the event and the organiser's `PendingUpload`. `GET ?page=N` renders the formset for rows `[(N-1)*25 : N*25]` of `mapped_rows`, pre-filled from their current values, each row's template display including its current `errors` (if any). `POST` handles three actions distinguished by which submit button fired: **Save & Next/Previous** — reads each row's submitted values directly off `request.POST` (not via `formset.is_valid()`) and re-validates them through the same `validate_row()` helper Phase 3 uses (blank room number, non-numeric capacity — field-level only), writes the (possibly corrected) values and any remaining per-row errors back into the corresponding slice of `mapped_rows`, redirects to the requested page. **Confirm** — first saves the current page same as above, then runs one pass over the *entire* `mapped_rows` list: if `mapped_rows` is empty (a header-only or genuinely empty CSV), block with a "No rows found in this CSV" message rather than allowing a silent zero-room save; if any row has a per-row error, or a normalized room number collides with another row, block with a message identifying the affected page number(s)/row(s) and do not proceed. If the full set is clean: inside `transaction.atomic()`, delete all of the event's existing `Room` rows (only relevant on a replace-flow re-upload) and `bulk_create()` new ones from `mapped_rows`, then delete the `PendingUpload`, then redirect to the event's edit page with a success message naming the room count created.
